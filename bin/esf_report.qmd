...
edisites$summary %>% 
  count(reason) %>% 
  mutate(reason = replace_na(reason, 'pass')) %>% 
   mutate(reasons = reason) %>% 
  separate_rows(reason, sep =';') %>% 
  group_by(reason) %>%
  mutate(nr = sum(n)) %>% 
  ungroup() %>% 
  arrange(nr) %>% 
  mutate(reason = factor(reason, unique(c('pass', reason))),
         prop = n / sum(n)) %>% 
  mutate(reasons = if_else(prop < 0.01, 'other', reasons)) %>% 
...
marg_rate <-
  edifit$anova %>% 
  filter(term == 'group', q_value_bh < 1e-5) %>% 
  select(site_id) %>% 
  left_join(
    edifit$margins,
    by = 'site_id'
